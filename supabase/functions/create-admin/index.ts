...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const adminExists = existingUsers?.users?.some(u => u.email === 'alexabraham587@gmail.com');

    if (adminExists) {
      // Find user and ensure admin role
      const adminUser = existingUsers!.users!.find(u => u.email === 'alexabraham587@gmail.com')!;
    ...
    const { data: newUser, error: createError } = await supabaseAdmin.auth.admin.createUser({
      ...
      email_confirm: true,
      user_metadata: { display_name: 'Abdullah' },
    });
